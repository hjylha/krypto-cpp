...
CodewordWordPair1::CodewordWordPair1(std::vector<int> the_codeword1, std::vector<int> the_codeword2, std::vector<int> the_word1, std::vector<int> the_word2) {
    codeword1 = the_codeword1;
    codeword2 = the_codeword2;
    word1 = the_word1;
    word2 = the_word2;
...
CodewordPuzzle1::CodewordPuzzle1() {

}

// CodewordPuzzle::CodewordPuzzle(std::vector<std::vector<int>> the_codewords, std::vector<std::string> the_wordlist, std::string the_alphabet, std::vector<std::string> the_comments) {
CodewordPuzzle1::CodewordPuzzle1(std::vector<std::vector<int>> the_codewords, std::vector<int> the_codeword_lengths, std::vector<std::vector<int>> the_wordlist, std::vector<int> the_word_lengths, int the_num_of_letters, std::vector<std::string> the_comments) {
    codewords = the_codewords;
    codeword_lengths = the_codeword_lengths;
    num_of_letters_in_alphabet = the_num_of_letters;

    wordlist = the_wordlist;
    word_lengths = the_word_lengths;
    comments = the_comments;
    ...
    for (int i = 0; i <= num_of_letters_in_alphabet; i++) {
        substitution_vector_opp.push_back(empty_num);
        substitution_vector_opp_saved.push_back(empty_num);
    } 

    for (int i = 0; i <= num_of_letters; i++) {
        substitution_vector.push_back(empty_num);
        substitution_vector_saved.push_back(empty_num);
    }
...
std::vector<std::vector<int>> CodewordPuzzle1::get_codewords() {
    return codewords;
}

std::vector<std::vector<int>> CodewordPuzzle1::get_wordlist() {
    return wordlist;
}

std::vector<std::string> CodewordPuzzle1::get_comments() {
    return comments;
}

std::vector<int> CodewordPuzzle1::get_substitution_vector() {
...
int CodewordPuzzle1::count_solved_numbers() {
    int num_of_solved_nums = 0;
    for (int i = 1; i <= num_of_letters; i++) {
        if (substitution_vector[i] != empty_num) {
            num_of_solved_nums++;
        }
    }
    return num_of_solved_nums;
...
void CodewordPuzzle1::save_substitution_vector() {
    for (int i = 0; i < num_of_codewords; i++) {
        substitution_vector_saved[i] = substitution_vector[i];
    }
    for (int i = 0; i < num_of_letters_in_alphabet; i++) {
        substitution_vector_opp_saved[i] = substitution_vector_opp[i];
    }
}

void CodewordPuzzle1::load_substitution_vector() {
    for (int i = 0; i < num_of_codewords; i++) {
        substitution_vector[i] = substitution_vector_saved[i];
    }
    for (int i = 0; i < num_of_letters_in_alphabet; i++) {
        substitution_vector_opp[i] = substitution_vector_opp_saved[i];
    }
    set_matched_words_from_start();
}

int CodewordPuzzle1::get_codeword_index(std::vector<int> codeword) {
    auto it = std::find(codewords.begin(), codewords.end(), codeword);
    if (it == codewords.end()) {
        return -1;
    }
...
bool CodewordPuzzle1::is_codeword_solved(std::vector<int> codeword) {
    for (int num : codeword) {
        if (substitution_vector[num] == empty_num) {
            return false;
        }
    }
    return true;
...
std::pair<int, int> CodewordPuzzle1::count_solved_codewords() {
    int num_of_solved_codewords = 0;
    int num_of_words_in_wordlist = 0;
    // std::string word;
    ...
    for (int i = 0; i < num_of_codewords; i++) {
        if (is_codeword_solved(codewords[i])) {
            num_of_solved_codewords++;
            word_vector = get_decrypted_codeword(codewords[i]);
            // word_vector = utf8_split(word);
            if (std::find(matched_words_all[i].begin(), matched_words_all[i].end(), word_vector) != matched_words_all[i].end()) {
                num_of_words_in_wordlist++;
            }
...
// bool CodewordPuzzle::does_word_match_to_substitution_vector(std::string word, std::vector<int> codeword) {
bool CodewordPuzzle1::does_word_match_to_substitution_vector(std::vector<int> word_vector, std::vector<int> codeword) {
    // std::vector<std::string> letters_in_substitution_vector = get_letters_in_substitution_vector();
    for (int i = 0; i < word_vector.size(); i++) {
        // if (substitution_vector[codeword[i]] != empty_symbol && substitution_vector[codeword[i]] != word[i]) {
        if (substitution_vector[codeword[i]] != empty_num && substitution_vector[codeword[i]] != word_vector[i]) {
            return false;
        }
        // bool condition = std::find(letters_in_substitution_vector.begin(), letters_in_substitution_vector.end(), word[i]) != letters_in_substitution_vector.end();
        // bool condition = std::find(letters_in_substitution_vector.begin(), letters_in_substitution_vector.end(), word_vector[i]) != letters_in_substitution_vector.end();
        if (substitution_vector[codeword[i]] == empty_num && substitution_vector_opp[word_vector[i]] != 0) {
            return false;
        }
    }
return true;
...
void CodewordPuzzle1::set_matched_words() {
    for (int i = 0; i < num_of_codewords; i++) {
        std::vector<std::vector<int>> new_matched_words;
        for (std::vector<int> word_vector : matched_words[i]) {
            if (does_word_match_to_substitution_vector(word_vector, codewords[i])) {
                new_matched_words.push_back(word_vector);
            }
        }
...
void CodewordPuzzle1::set_matched_words_from_start() {
    for (int i = 0; i < num_of_codewords; i++) {
        std::vector<std::vector<int>> new_matched_words;
        for (std::vector<int> word_vector : matched_words_all[i]) {
            if (does_word_match_to_substitution_vector(word_vector, codewords[i])) {
                new_matched_words.push_back(word_vector);
            }
        }
...
// std::vector<std::string> CodewordPuzzle::get_matched_words_for_codeword(int index_of_codeword) {
std::vector<std::vector<int>> CodewordPuzzle1::get_matched_words_for_codeword(int index_of_codeword) {
    return matched_words[index_of_codeword];
}

int CodewordPuzzle1::get_num_of_matched_words(int index_of_codeword) {
    return matched_words[index_of_codeword].size();
}

std::vector<int> CodewordPuzzle1::sort_codewords() {
    std::vector<int> sorted_indices;
    std::vector<int> comparison;
    // int comparison_num;
    for (int i = 0; i < num_of_codewords; i++) {
...
void CodewordPuzzle1::clear_substitution_vector() {
    for (int i = 0; i < substitution_vector.size(); i++) {
...
int CodewordPuzzle1::add_to_substitution_vector(int num, int letter, std::map<std::string, int> issues, bool override) {
    if (num <= 0 || num >= substitution_vector.size()) {
        return issues["invalid number"];
    }
    if (letter == empty_num) {
        substitution_vector[num] = empty_num;
        // std::cout << "Added in CodewordPuzzle " << num << " = " << empty_symbol << std::endl;
        set_matched_words_from_start();
        return 0;
    }
    ...
    // letter = lowercase(letter);
    // if (alphabet.find(letter) == std::string::npos) {
    if (letter < 0 || letter > num_of_letters_in_alphabet) {
        return issues["invalid letter"];
    }
    int previous_num = substitution_vector_opp[letter];
    if (previous_num != empty_num && !override) {
        return issues["double letter"];
    }
    if (previous_num != -1 && override) {
        substitution_vector[previous_num] = empty_num;
    }
    substitution_vector[num] = letter;
    substitution_vector_opp[letter] = num;
    // std::cout << "Added in CodewordPuzzle " << num << " = " << letter << std::endl;
    // std::cout << "Confirmation: " << num << " = " << substitution_vector[num] << std::endl;
    // std::cout << "Confirmation2: " << find_char_from_substitution_vector(letter) << " = " << letter << std::endl;
    set_matched_words();
...
void CodewordPuzzle1::set_codeword_to_word(int codeword_index, std::vector<int> word_vector) {
    // TODO: REWORK NEEDED
    for (int i = 0; i < word_vector.size(); i++) {
        // add_to_substitution_vector(codewords[codeword_index][i], word[i], std::map<std::string, int>(), true);
        add_to_substitution_vector(codewords[codeword_index][i], word_vector[i], std::map<std::string, int>(), true);
...
// int CodewordPuzzle1::find_char_from_substitution_vector(std::string letter) {
//     auto it = std::find(substitution_vector.begin(), substitution_vector.end(), letter);
//     if (it == substitution_vector.end()) {
//         return -1;
//     }
...
std::vector<int> CodewordPuzzle1::find_codeword(std::string search_string) {
    std::vector<int> search_result;
...
std::vector<int> CodewordPuzzle1::find_codeword_with_least_matches() {
    std::vector<int> the_codeword;
    int least_matches = wordlist.size();
    for (int i = 0; i < num_of_codewords; i++) {
        if (is_codeword_solved(codewords[i])) continue;
...
std::vector<int> CodewordPuzzle1::get_decrypted_codeword(std::vector<int> codeword) {
    std::vector<int> decrypted_codeword;
    for (int i = 0; i < codeword.size(); i++) {
        decrypted_codeword.push_back(substitution_vector[codeword[i]]);
    }
    return decrypted_codeword;
}

// std::vector<std::pair<std::string, std::string>> CodewordPuzzle::match_two_codewords(int codeword_index1, int codeword_index2, int maximum_matches) {
std::vector<std::pair<std::vector<int>, std::vector<int>>> CodewordPuzzle1::match_two_codewords(int codeword_index1, int codeword_index2, int maximum_matches) {
...
std::vector<CodewordWordPair1> CodewordPuzzle1::find_all_unique_pairs() {
...
std::pair<int, std::vector<int>> CodewordPuzzle1::find_optimal_match() {
    std::vector<CodewordWordPair1> unique_pairs = find_all_unique_pairs();
    if (unique_pairs.empty()) {
        return std::pair<int, std::vector<int>>(-1, {});
    }
    // std::map<std::pair<int, std::string>, int> num_of_appearances;
    std::vector<int> num_of_appearances;
    std::vector<std::pair<int, std::vector<int>>> matches;
    std::vector<int> codeword1, codeword2;
    std::vector<int> word1, word2;
    // std::vector<std::string> word1, word2;
    int codeword1_index, codeword2_index;
    int index = -1;
    for (CodewordWordPair1 cw_w_pair : unique_pairs) {
        codeword1 = cw_w_pair.codeword1;
        codeword1_index = std::find(codewords.begin(), codewords.end(), codeword1) - codewords.begin();
        word1 = cw_w_pair.word1;
        // word1 = join_string(cw_w_pair.word1, "");
        if (!is_codeword_solved(codeword1)) {
            index = -1;
            for (int i = 0; i < matches.size(); i++) {
                if (matches[i].first == codeword1_index && matches[i].second == word1) {
                    index = i;
                }
            }
            if (index == -1) {
                num_of_appearances.push_back(1);
                matches.push_back(std::pair<int, std::vector<int>>(codeword1_index, word1));
            }
            else {
                num_of_appearances[index] += 1;
            }
        ...
        word2 = cw_w_pair.word2;
        // word2 = join_string(cw_w_pair.word2, "");
        codeword2_index = std::find(codewords.begin(), codewords.end(), codeword2) - codewords.begin();
        
        if (!is_codeword_solved(codeword2)) {
            index = -1;
            for (int i = 0; i < matches.size(); i++) {
                if (matches[i].first == codeword2_index && matches[i].second == word2) {
                    index = i;
                }
            }
            if (index == -1) {
                num_of_appearances.push_back(1);
                matches.push_back(std::pair<int, std::vector<int>>(codeword2_index, word2));
            }
            else {
                num_of_appearances[index] += 1;
            }
    ...
    std::pair<int, std::vector<int>> optimal_match;
    // std::pair<int, std::vector<int>> match;
    int max_num_of_appearances = 0;
    for (int i = 0; i < num_of_appearances.size(); i++) {
        // match = matches[i];
        if (num_of_appearances[i] > max_num_of_appearances) {
            optimal_match = matches[i];
            max_num_of_appearances = num_of_appearances[i];
        }
    }
